...
export default function PropFlow() {
  ...
  const [contractorForm, setContractorForm] = useState({ name: '', type: '', phone: '', email: '' })
  ...
  const callAI = async (userMsg) => {
    const ctx = `Portfolio: ${data.properties.length} properties, ${data.tenants.length} tenants. Monthly rent: ${fmt(totalRent)}. Expenses: ${fmt(totalExpenses)}. Owner payouts: ${fmt(totalPayouts)}. Late tenants: ${lateCount}. Vacant: ${vacantCount}. Open maintenance: ${openMaintenance}. Mgmt fee: ${data.mgmtFeePercent}%.`
    ...
    try {
      const res = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          prompt: userMsg,
          systemPrompt: `You are PropFlow AI, a smart property management assistant. Be concise and practical. Write in plain conversational text only — no markdown, no asterisks, no bullet dashes, no tables, no pound signs. Just clear natural paragraphs. Context: ${ctx}`
        })
      })
  ...
  const downloadPDF = async (property) => {
    ...
    doc.text('PropFlow', 20, 22)
    doc.setFontSize(11); doc.setFont('helvetica', 'normal')
    doc.text('Owner Statement', 20, 30)
    doc.line(20, 35, 190, 35)
    doc.text(`Property: ${property.name}`, 20, 44)
    doc.text(`Owner: ${property.owner}  |  ${property.ownerEmail}`, 20, 51)
    doc.text(`Period: ${data.currentMonth}`, 20, 58)
    doc.line(20, 63, 190, 63)
    let y = 72
    doc.text('Rent Collected:', 20, y); doc.text(fmt(property.rent), 160, y, { align: 'right' }); y += 8
    propExpenses.forEach(e => { doc.setTextColor(150); doc.text(`  ${e.description}`, 20, y); doc.text(`-${fmt(e.amount)}`, 160, y, { align: 'right' }); doc.setTextColor(0); y += 7 })
    doc.text(`Management Fee (${data.mgmtFeePercent}%):`, 20, y); doc.setTextColor(150); doc.text(`-${fmt(mgmtFeeAmt)}`, 160, y, { align: 'right' }); doc.setTextColor(0); y += 8
    doc.line(20, y, 190, y); y += 8
    doc.setFont('helvetica', 'bold'); doc.setFontSize(13)
    doc.text('Owner Payout:', 20, y); doc.text(fmt(ownerPayout), 160, y, { align: 'right' })
    doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(150)
    doc.text('PropFlow is not a licensed accounting service. Consult a CPA for tax filing.', 20, 280)
    ...
    <>
      ...
      <div className="app-layout">
        <aside className="sidebar">
            ...
            <div className="nav-section">
              ...
              {NAV.map(n => (
                <button key={n.id} className={`nav-item${page === n.id ? ' active' : ''}`} onClick={() => setPage(n.id)}>
                  ...
                  {n.id === 'maintenance' && openMaintenance > 0 && <span className="badge yellow" style={{ marginLeft: 'auto', fontSize: 10 }}>{openMaintenance}</span>}
          ...
          <div className="sidebar-footer">
            <div className="user-pill">
              ...
              <div><div className="user-name">Ryan Hammond</div><div className="user-plan">Pro Plan</div></div>
            </div>
          </div>
        </aside>
        ...
        <div className="main-content">
          ...
          {page === 'dashboard' && (
            <div className="page">
              <div className="page-header">
                ...
                <div className="page-sub">{data.currentMonth} portfolio overview</div>
              </div>
              <div className="stats-grid">
                ...
                <div className="stat-card"><div className="stat-label">📈 Net Income</div><div className="stat-value text-green">{fmt(netIncome)}</div><div className="stat-change up">after expenses</div></div>
              ...
              {(lateCount > 0 || vacantCount > 0 || openMaintenance > 0) && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: '1.5rem' }}>
                  {lateCount > 0 && <div style={{ background: 'var(--red-bg)', border: '1px solid #fecaca', borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 13, color: 'var(--red)', fontWeight: 600 }}>⚠️ {lateCount} tenant{lateCount > 1 ? 's' : ''} with late payment</span><button className="sm danger" onClick={() => setPage('tenants')}>View →</button></div>}
                  {openMaintenance > 0 && <div style={{ background: 'var(--yellow-bg)', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 13, color: 'var(--yellow)', fontWeight: 600 }}>🔧 {openMaintenance} open maintenance request{openMaintenance > 1 ? 's' : ''}</span><button className="sm" onClick={() => setPage('maintenance')}>View →</button></div>}
                  {vacantCount > 0 && <div style={{ background: 'var(--yellow-bg)', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 13, color: 'var(--yellow)', fontWeight: 600 }}>🏠 {vacantCount} vacant unit{vacantCount > 1 ? 's' : ''}</span><button className="sm" onClick={() => setPage('properties')}>View →</button></div>}
                </div>
              )}
                ...
                <div className="card-body">
                  {expenseByCategory.map(c => (
                    <div key={c.cat} style={{ marginBottom: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}><span>{c.cat}</span><span className="text-muted">{fmt(c.total)}</span></div>
                      <div className="progress-bar"><div className="progress-fill" style={{ width: `${(c.total / totalExpenses) * 100}%` }} /></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="page">
              ...
              <div style={{ marginBottom: '1rem' }}>
                <div className="progress-bar" style={{ height: 10, borderRadius: 5 }}>
                  <div className="progress-fill" style={{ width: `${Math.min((rentCollected / totalRent) * 100, 100)}%` }} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text2)', marginTop: 4 }}>
                  <span>{fmt(rentCollected)} collected</span><span>{fmt(totalRent - rentCollected)} outstanding</span>
                </div>
              </div>
              ...
              <div className="card">
                <div className="card-header"><span className="card-title">Payment Log</span></div>
                <div className="table-wrap"><table>
                  <thead><tr><th>Tenant</th><th>Property</th><th>Amount</th><th>Date Paid</th><th>Method</th><th>Status</th></tr></thead>
                  <tbody>
                    {data.tenants.map(t => {
                      ...
                      return <tr key={t.id}>
                        ...
                        <td>{getProperty(t.propertyId)?.name}</td>
                        <td>{fmt(t.rentAmount)}</td>
                        <td>{payment ? payment.datePaid : <span className="text-muted">—</span>}</td>
                        <td>{payment ? payment.method : <span className="text-muted">—</span>}</td>
                        <td>{payment ? <span className="badge green">✓ Paid</span> : t.paymentStatus === 'late' ? <span className="badge red">Late</span> : <span className="badge yellow">Pending</span>}</td>
                      </tr>
                    })}
                  </tbody>
                </table></div>
          ...
          {page === 'maintenance' && (
            <div className="page">
              <div className="page-header"><div className="page-title">Maintenance Requests</div><div className="page-sub">{openMaintenance} open · {data.maintenance.filter(m => m.status === 'resolved').length} resolved</div></div>
              <div className="card"><div className="table-wrap"><table>
              ...
              <div className="card">
                <div className="card-header"><span className="card-title">Contractor Payments YTD</span>
                  <button className="primary sm" onClick={() => {
                    ...
                    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = '1099-report-2026.csv'; a.click()
                  }}>⬇ Export CSV</button>
                </div>
                <div className="table-wrap"><table>
                ...
                <div style={{ padding: '0.75rem 1.25rem', borderTop: '1px solid var(--border)', fontSize: 12, color: 'var(--text2)' }}>⚠️ 1099-NEC required for contractors paid $600+ per calendar year. Consult your CPA for filing deadlines.</div>
              </div>
            </div>
          )}
            ...
            <div className="page">
              ...
              <div className="grid-2">
                <div className="card">
                  ...
                  <div className="card-body">
                    {[
                      { label: 'Gross Rent Collected', val: totalRent, positive: true },
                      { label: 'Total Expenses', val: -totalExpenses, positive: false },
                      { label: 'Net Operating Income', val: netIncome, positive: netIncome > 0, bold: true },
                      { label: 'Management Fees (your income)', val: totalMgmt, positive: true, bold: true },
                      { label: 'Owner Payouts', val: -totalPayouts, positive: false },
                    ].map((r, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: r.bold ? 14 : 13, fontWeight: r.bold ? 700 : 400 }}>
                      <span>{r.label}</span>
                      <span className={r.positive ? 'text-green' : 'text-red'}>{r.val < 0 ? `−${fmt(Math.abs(r.val))}` : fmt(r.val)}</span>
                    </div>)}
                  </div>
                </div>
                <div className="card">
                  ...
                  <div className="card-body">
                    {[
                      ...
                      { label: 'Total Contractors', val: data.contractors.length },
                      { label: 'Deposits Held', val: null, formatted: fmt(data.tenants.reduce((s, t) => s + (t.depositHeld || 0), 0)) },
                    ].map((r, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
              ...
              <div className="card" style={{ marginTop: '1rem' }}>
                <div className="card-header"><span className="card-title">Owner Payout Summary</span>
                  <button className="primary sm" onClick={() => {
                    ...
                    const blob = new Blob([csv], { type: 'text/csv' })
                    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `payout-summary-${data.currentMonth.replace(/\s+/g, '-')}.csv`; a.click()
                  }}>⬇ Export CSV</button>
                </div>
                <div className="table-wrap"><table>
          ...
          {page === 'ai' && (
            <div className="ai-panel">
              ...
              <div className="ai-messages">
                ...
                {aiLoading && <div className="ai-msg assistant"><div className="ai-avatar">✨</div><div className="ai-bubble"><span className="spinner dark" /></div></div>}
      ...
      {payoutModal && (() => {
        ...
        return <div className="modal-overlay" onClick={() => setPayoutModal(null)}><div className="modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="modal-body">
            <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: '1rem' }}>{payoutModal.owner} · {payoutModal.ownerEmail} · {data.currentMonth}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}><span>Rent collected</span><strong>{fmt(payoutModal.rent)}</strong></div>
            {propExpenses.map((e, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}><span className="text-muted">— {e.description}</span><span className="text-red">−{fmt(e.amount)}</span></div>)}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '2px solid var(--border)', fontSize: 13 }}><span className="text-muted">Management fee ({data.mgmtFeePercent}%)</span><span className="text-red">−{fmt(mgmtFeeAmt)}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', fontSize: 16, fontWeight: 800 }}><span>Owner Payout</span><span className="text-green">{fmt(ownerPayout)}</span></div>
            <p style={{ fontSize: 11, color: 'var(--text2)', marginTop: 4 }}>PropFlow is not a licensed accounting service. Consult a CPA for tax filing.</p>
          </div>
      ...
      {modal === 'expense' && <div className="modal-overlay" onClick={() => setModal(null)}><div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          <button className="primary" onClick={() => { setData(d => ({ ...d, expenses: [...d.expenses, { ...expenseForm, id: Date.now(), propertyId: parseInt(expenseForm.propertyId), contractorId: parseInt(expenseForm.contractorId), amount: parseFloat(expenseForm.amount) || 0 }] })); setModal(null); setExpenseForm({ propertyId: '', contractorId: '', description: '', amount: '', date: '', category: 'Maintenance' }) }}>Add Expense</button>
        </div>
      </div></div>}
      ...
      {modal === 'property' && <div className="modal-overlay" onClick={() => setModal(null)}><div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          <button onClick={() => setModal(null)}>Cancel</button>
          <button className="primary" onClick={() => { setData(d => ({ ...d, properties: [...d.properties, { ...propertyForm, id: Date.now(), rent: parseFloat(propertyForm.rent) || 0 }] })); setModal(null); setPropertyForm({ name: '', address: '', owner: '', ownerEmail: '', rent: '', status: 'occupied', tenant: '' }) }}>Add Property</button>
        </div>
      </div></div>}
      ...
      {modal === 'maintenance' && <div className="modal-overlay" onClick={() => setModal(null)}><div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          <button onClick={() => setModal(null)}>Cancel</button>
          <button className="primary" onClick={() => { setData(d => ({ ...d, maintenance: [...d.maintenance, { ...maintenanceForm, id: Date.now(), propertyId: parseInt(maintenanceForm.propertyId), contractorId: maintenanceForm.contractorId ? parseInt(maintenanceForm.contractorId) : null, status: 'open', dateReported: new Date().toISOString().split('T')[0], dateResolved: null }] })); setModal(null); setMaintenanceForm({ propertyId: '', tenantName: '', issue: '', priority: 'medium', contractorId: '', notes: '' }) }}>Log Issue</button>
        </div>
      </div></div>}
      ...
      {modal === 'rent' && <div className="modal-overlay" onClick={() => setModal(null)}><div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          <button className="primary" onClick={() => {
            ...
            setData(d => ({
              ...d,
              rentPayments: [...d.rentPayments, { ...rentForm, id: Date.now(), tenantId: parseInt(rentForm.tenantId), propertyId: tenant.propertyId, amount: parseFloat(rentForm.amount) || 0, status: 'paid' }],
              tenants: d.tenants.map(t => t.id === parseInt(rentForm.tenantId) ? { ...t, paymentStatus: 'paid', daysLate: 0 } : t)
            }))
            setModal(null)
            setRentForm({ tenantId: '', amount: '', datePaid: '', method: 'ACH', month: data.currentMonth })
